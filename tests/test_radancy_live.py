"""
NOTE: this script touches the real internet (public Radancy/TalentBrew
...
from scheduler import scrape_radancy_live

if __name__ == "__main__":
    config = load_config()
    radancy_companies = [c for c in config["companies"] if c.get("radancy_base_url")]

    jobs = scrape_radancy_live(config)

    assert len(jobs) > 0, "expected at least some jobs"
    seen_sources = {job["source"] for job in jobs}
    print(f"{len(jobs)} unique jobs fetched from {len(seen_sources)}/{len(radancy_companies)} companies")

    missing = {c["name"] for c in radancy_companies} - seen_sources
    if missing:
